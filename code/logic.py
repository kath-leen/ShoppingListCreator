...
def chooseRecipes(recipes_database):
    ...
    desired_recipes_by_id_str = input('\nPlease choose what recipes do you like to add (print IDs, divide by comma, do not use space) ')
    desired_recipes_by_id = desired_recipes_by_id_str.split(',')
    return desired_recipes_by_id

def summarizeAllIngredients(desired_recipes_by_id, recipes_ingredients_database):
    ...
    for recipe_id in desired_recipes_by_id:
        ...
        for recipe_ingredient_data in recipe_ingredients_data:
            ingredients_summary_data[recipe_ingredient_data.ingredient_id] = ingredients_summary_data.get(recipe_ingredient_data.ingredient_id, 0) + recipe_ingredient_data.quantity
    return ingredients_summary_data

def printAllSummarizedIngredients(ingredients_summary_data, ingredients_database):
    ...
    for ingredient_id in list(ingredients_summary_data):
        ...
        print('Ingredient: ' + ingredient.name + ', quantity: ' + str(ingredients_summary_data[ingredient_id]) + ' ' + ingredient.meas_unit)
